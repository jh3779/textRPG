 ...
 * DEC-125로 4개 변종 포트레이트 중 랜덤 선택되도록 확장됐지만 스탯 고정 원칙은 그대로). 반면 고블린은
 * DEC-124로 변종별 스탯이 서로 다르며, 이 필드가 그 스탯 선택과 함께 확정된 결과(단일 파일명 1개)를
 * 담을 뿐 — 즉 "이 필드 자체가 스탯을 바꾸지는 않는다"는 여전히 사실이지만, 옛 ASM-104 가정("고블린도
 * 그림만 다름")은 더 이상 유효하지 않다(GameSession.StartBattleWithGoblin 참조).
 */
...
namespace TextRPG.GameLogic
{
    public class Enemy
    {
        ...
        /// <summary>
        /// 신규(DEC-114): 전투 시작 시 표시할 초상화 파일명. 던전 수호자는 시각만 다르고 스탯은 균형형
        /// 그대로 고정된 채, 이 배열은 GameSession.StartBattleWithGuardian()이 조우 시점에 4개 변종
        /// (균형형·중장형·기동형·마도형) 중 랜덤으로 고른 단일 확정 포트레이트 1개만 담는다
        /// (DEC-125, OQ-108 해결 — 스탯과는 여전히 무관, "회차마다 랜덤 보스" = 조우 시점 랜덤 선택).
